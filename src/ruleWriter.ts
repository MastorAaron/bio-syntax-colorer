...
import { LangFile, DeconFile, ColorFile, JsonFile, Lang, RuleType, FilePath, FileMeta } from "./fileMeta";
...
export abstract class RuleWriter{
    ...
    protected regi = new RegExBuilder(true); 
    
constructor(protected context: vscode.ExtensionContext, meta : FileMeta){
        
        this.lang = meta.lang;
        this.JSONType = meta.jsonKind;
        this.patcher  = new PatchColors(context,meta);
    ...
    protected genPatternNameScope(letter : string, tokenType : string=""): def.GenericScope {
       // Resolve symbol alias if it's a known special symbol, otherwise fallback to original letter
        ...
        const name = `${source}${this.lang}.${token}`;
